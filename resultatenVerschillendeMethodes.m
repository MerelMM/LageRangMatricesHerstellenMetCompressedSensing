...
for j=start:aantalMetingen
    ...
    for i = 1:aantalKeer
        ...
        ipen=ipen+itpen(k);
        igradEnPen = igradEnPen + itgradEnPen(k);
        if (gewoon(k)<=drempel)
            sgew = sgew + 1;
        end
...
fig7 = figure(7)
plot(x,succesRecoveryGradEnPen,'--ks')
title('met penalty en gradiënt',fontsize=16)
xlabel('oversamplingsfactor \phi [aantal metingen / (m+n-rang)*rang]',fontsize=16)
ylabel('succesvol hersteld [%]',fontsize=16)
grid on

fig8 = figure(8)
plot(x,gemItGradEnPen,'--ks')
title('gemiddeld aantal iteraties',fontsize=16)
xlabel('oversamplingsfactor \phi [aantal metingen / (m+n-rang)*rang]',fontsize=16)
ylabel('succesvol hersteld [%]',fontsize=16)
grid on