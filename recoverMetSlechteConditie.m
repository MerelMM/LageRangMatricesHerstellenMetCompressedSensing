%wanneer we een rang 5 matrix willen benaderen met een rang 4 matrix en de
%laatste 2 singuliere waarden dicht bij elkaar lagen 
function[x] = recoverMetSlechteConditie(C,meting,rijen, kolommen, rang)
lambda = 10;
rang = rang -1;
A = randn(rijen,rang);
...
objfun = @(z)recoverWithGradient(C,z, kolommen, rijen, rang, meting, lambda)
[f,g] = objfun(z0)
g2 = g*0;
for i = 1:length(g)
    zh=z0;
...
function[f]= recoverWithGradientt(C,z, kolommen, rijen, rang, meting, lambda)
...
function[f,gradientt] = recoverWithGradient(C,z, kolommen, rijen, rang, meting, lambda)
