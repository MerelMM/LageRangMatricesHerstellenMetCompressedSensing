function [x] = recoverWithGradient(rijen, kolommen, rang, aantalMetingen, lambda)
A = randn(rijen,rang);
...
[z] = fminunc(@(z)recoverWGradient(C,z, kolommen, rijen, rang, meting, lambda),z0,options);
...
function[f,gradient] = recoverWGradient(C,z, kolommen, rijen, rang, meting, lambda)
f = 0.5*norm(C * reshape((reshape(z(1:rijen*rang,:), rijen, rang) ...
    *reshape(z(rijen*rang+1:(rijen*rang+kolommen*rang),:),rang,kolommen)),rijen*kolommen,1)-meting)^2 + ...
    lambda * (norm(reshape(z(1:rijen*rang,:), rijen, rang))^2 - norm(reshape(z(rijen*rang+1:(rijen*rang+kolommen*rang),:),rang,kolommen))^2)^2;
